defmodule GearLibraryApiWeb.LibraryControllerTest do
  ...
  describe "create library" do
    test "renders library when data is valid", %{conn: conn} do
      conn = post(conn, ~p"/api/libraries", library: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, ~p"/api/libraries/#{id}")
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, ~p"/api/libraries", library: @invalid_attrs)
  ...
  describe "update library" do
    ...
    test "renders library when data is valid", %{conn: conn, library: %Library{id: id} = library} do
      conn = put(conn, ~p"/api/libraries/#{library}", library: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, library: library} do
      conn = put(conn, ~p"/api/libraries/#{library}", library: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete library" do
    ...
    test "deletes chosen library", %{conn: conn, library: library} do
      conn = delete(conn, ~p"/api/libraries/#{library}")
      assert response(conn, 204)

      assert_error_sent 404, fn ->
        get(conn, ~p"/api/libraries/#{library}")
      end
    end
  end
